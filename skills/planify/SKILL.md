...
- **Red light:** Failing tests for this task’s behaviour(s). List the tests you will create.
- **Green light:** Minimal implementation to make those tests pass. List the implementation details.
- **Refactor:** Improve the code while keeping tests green.

Tasks are vertical slices: each task groups related behaviour and gets its own Red / Green / Refactor steps.
...
4. **Build the plan.**
   ...
   - For each task, read **assets/task-structure-template.md** and fill it with implementation-ready detail:
     ...
     - **Red light (tests first):**
       - List the failing tests to add for this task's behaviours, including key happy path and relevant edge/error cases.
     ...
     - **Green light (minimal implementation):**
       ...
       - Name important modules, components, handlers, schemas, state updates, or data-flow touchpoints involved.
       - Keep bullets sequential and action-oriented so someone can execute them directly.
